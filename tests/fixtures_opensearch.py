"""Synthetic OpenSearch API responses with known ground truth.

All values are fabricated; figures derived from them are asserted exactly in
tests (e.g. 3 data nodes, 100 active shards, 4.0 GB/day ingestion).
"""

from __future__ import annotations

from datetime import UTC, datetime, timedelta

# ── ground-truth constants ──────────────────────────────────────────────

NUM_NODES = 3
NUM_DATA_NODES = 3
ACTIVE_SHARDS = 100
CLUSTER_NAME = "os-test-cluster"
OS_VERSION = "2.11.0"

# index data: 10 date-based indices over 5 days, each 2 GB primary
NUM_DATE_INDICES = 10
PRI_BYTES_PER_INDEX = 2_000_000_000  # 2 GB
REPLICAS = 1  # store.size = pri * (1 + replicas)
DOCS_PER_INDEX = 1_000_000
DAYS_WITH_DATA = 5
LOOKBACK_DAYS = 7

# derived ground truth
TOTAL_DATE_DOCS = NUM_DATE_INDICES * DOCS_PER_INDEX  # 10M
TOTAL_STORE_BYTES = NUM_DATE_INDICES * PRI_BYTES_PER_INDEX * (1 + REPLICAS)  # 40 GB
TOTAL_PRI_BYTES = NUM_DATE_INDICES * PRI_BYTES_PER_INDEX  # 20 GB
INGEST_GB_PER_DAY = TOTAL_PRI_BYTES / DAYS_WITH_DATA / 1e9  # 4.0 GB/day

# OTel trace indices: 5 days, 1 index per day, 500 MB primary each, 500K spans each
NUM_TRACE_INDICES = 5
TRACE_PRI_BYTES_PER_INDEX = 500_000_000  # 500 MB
TRACE_DOCS_PER_INDEX = 500_000
TRACE_REPLICAS = 1
TRACE_DAYS_WITH_DATA = 5

TRACE_TOTAL_DOCS = NUM_TRACE_INDICES * TRACE_DOCS_PER_INDEX  # 2.5M spans
TRACE_TOTAL_STORE = NUM_TRACE_INDICES * TRACE_PRI_BYTES_PER_INDEX * (1 + TRACE_REPLICAS)  # 5 GB
TRACE_TOTAL_PRI = NUM_TRACE_INDICES * TRACE_PRI_BYTES_PER_INDEX  # 2.5 GB
TRACE_INGEST_GB_PER_DAY = TRACE_TOTAL_PRI / TRACE_DAYS_WITH_DATA / 1e9  # 0.5 GB/day
TRACE_SPANS_PER_DAY = TRACE_TOTAL_DOCS / TRACE_DAYS_WITH_DATA  # 500K spans/day

# system index (excluded from ingestion calc, included in totals)
SYSTEM_DOCS = 500
SYSTEM_STORE_BYTES = 50_000_000  # 50 MB
SYSTEM_PRI_BYTES = 25_000_000

GRAND_TOTAL_DOCS = TOTAL_DATE_DOCS + TRACE_TOTAL_DOCS + SYSTEM_DOCS
GRAND_TOTAL_STORE = TOTAL_STORE_BYTES + TRACE_TOTAL_STORE + SYSTEM_STORE_BYTES
GRAND_TOTAL_PRI = TOTAL_PRI_BYTES + TRACE_TOTAL_PRI + SYSTEM_PRI_BYTES


# ── helpers ─────────────────────────────────────────────────────────────


def _recent_dates(days: int = DAYS_WITH_DATA) -> list[str]:
    end = datetime.now(UTC).replace(hour=0, minute=0, second=0, microsecond=0)
    return [
        (end - timedelta(days=d)).strftime("%Y.%m.%d") for d in range(days, 0, -1)
    ]


def _make_date_indices() -> list[dict]:
    dates = _recent_dates()
    indices = []
    idx_per_day = NUM_DATE_INDICES // DAYS_WITH_DATA
    for day_date in dates:
        for j in range(idx_per_day):
            name = f"logs-app-{day_date}" if j == 0 else f"metrics-infra-{day_date}"
            indices.append({
                "index": name,
                "health": "green",
                "status": "open",
                "pri": "5",
                "rep": str(REPLICAS),
                "docs.count": str(DOCS_PER_INDEX),
                "docs.deleted": "0",
                "store.size": str(PRI_BYTES_PER_INDEX * (1 + REPLICAS)),
                "pri.store.size": str(PRI_BYTES_PER_INDEX),
            })
    return indices


def _make_trace_indices() -> list[dict]:
    dates = _recent_dates(TRACE_DAYS_WITH_DATA)
    indices = []
    for day_date in dates:
        indices.append({
            "index": f"otel-v1-apm-span-{day_date}",
            "health": "green",
            "status": "open",
            "pri": "2",
            "rep": str(TRACE_REPLICAS),
            "docs.count": str(TRACE_DOCS_PER_INDEX),
            "docs.deleted": "0",
            "store.size": str(TRACE_PRI_BYTES_PER_INDEX * (1 + TRACE_REPLICAS)),
            "pri.store.size": str(TRACE_PRI_BYTES_PER_INDEX),
        })
    return indices


def _make_system_index() -> dict:
    return {
        "index": ".opensearch-observability",
        "health": "green",
        "status": "open",
        "pri": "1",
        "rep": "1",
        "docs.count": str(SYSTEM_DOCS),
        "docs.deleted": "10",
        "store.size": str(SYSTEM_STORE_BYTES),
        "pri.store.size": str(SYSTEM_PRI_BYTES),
    }


# ── API response fixtures ──────────────────────────────────────────────


CLUSTER_HEALTH = {
    "cluster_name": CLUSTER_NAME,
    "status": "green",
    "number_of_nodes": NUM_NODES,
    "number_of_data_nodes": NUM_DATA_NODES,
    "active_primary_shards": 50,
    "active_shards": ACTIVE_SHARDS,
    "unassigned_shards": 0,
    "relocating_shards": 0,
    "initializing_shards": 0,
}

CLUSTER_STATS = {
    "cluster_name": CLUSTER_NAME,
    "cluster_uuid": "os-test-uuid-456",
    "nodes": {
        "count": {"total": NUM_NODES, "data": NUM_DATA_NODES},
        "versions": [OS_VERSION],
        "os": {"mem": {"total_in_bytes": 48_000_000_000}},
        "fs": {
            "total_in_bytes": 3_000_000_000_000,
            "available_in_bytes": 2_000_000_000_000,
        },
        "jvm": {"mem": {"heap_max_in_bytes": 24_000_000_000}},
    },
}


def _make_node(nid: str, name: str) -> dict:
    return {
        "name": name,
        "roles": ["data", "ingest", "cluster_manager"],
        "os": {
            "mem": {"total_in_bytes": 16_000_000_000, "used_percent": 72},
            "available_processors": 8,
        },
        "jvm": {
            "mem": {
                "heap_max_in_bytes": 8_000_000_000,
                "heap_used_in_bytes": 4_000_000_000,
            },
            "uptime_in_millis": 86_400_000,
            "start_time_in_millis": int(
                (datetime.now(UTC) - timedelta(days=1)).timestamp() * 1000
            ),
        },
        "fs": {
            "total": {
                "total_in_bytes": 1_000_000_000_000,
                "available_in_bytes": 600_000_000_000,
            },
            "data": [
                {
                    "path": "/data/opensearch",
                    "mount": "/data",
                    "type": "ext4",
                    "total_in_bytes": 1_000_000_000_000,
                    "available_in_bytes": 600_000_000_000,
                }
            ],
        },
        "indices": {
            "search": {
                "query_total": 50000,
                "query_time_in_millis": 250000,
                "fetch_total": 30000,
                "fetch_time_in_millis": 90000,
                "scroll_total": 1000,
                "scroll_time_in_millis": 5000,
                "suggest_total": 0,
                "suggest_time_in_millis": 0,
            }
        },
    }


NODES_STATS = {
    "_nodes": {"total": NUM_NODES, "successful": NUM_NODES},
    "nodes": {
        f"node_{i}": _make_node(f"node_{i}", f"os-data-{i}")
        for i in range(NUM_NODES)
    },
}

NODES_INFO = {
    "_nodes": {"total": NUM_NODES, "successful": NUM_NODES},
    "nodes": {
        f"node_{i}": {
            "name": f"os-data-{i}",
            "roles": ["data", "ingest", "cluster_manager"],
            "os": {"available_processors": 8},
            "jvm": {"version": "17.0.9"},
        }
        for i in range(NUM_NODES)
    },
}


def cat_indices() -> list[dict]:
    return _make_date_indices() + _make_trace_indices() + [_make_system_index()]


def index_settings() -> dict:
    indices = cat_indices()
    now = datetime.now(UTC)
    settings = {}
    for idx in indices:
        name = idx["index"]
        import re
        m = re.search(r"(\d{4})\.(\d{2})\.(\d{2})", name)
        if m:
            date_str = f"{m.group(1)}-{m.group(2)}-{m.group(3)}"
            dt = datetime.strptime(date_str, "%Y-%m-%d").replace(tzinfo=UTC)
        else:
            dt = now - timedelta(days=30)
        ts_ms = int(dt.timestamp() * 1000)
        settings[name] = {
            "settings": {"index": {"creation_date": str(ts_ms)}}
        }
    return settings


CLUSTER_INDEX_STATS = {
    "_all": {
        "total": {
            "search": {
                "query_total": 150000,
                "query_time_in_millis": 750000,
                "fetch_total": 90000,
                "fetch_time_in_millis": 270000,
                "scroll_total": 3000,
                "scroll_time_in_millis": 15000,
                "suggest_total": 0,
                "suggest_time_in_millis": 0,
            }
        }
    }
}

ISM_POLICIES = {
    "policies": [
        {
            "_id": "logs-policy",
            "policy": {
                "policy_id": "logs-policy",
                "description": "Log index lifecycle",
                "states": [
                    {
                        "name": "hot",
                        "actions": [
                            {"rollover": {"min_size": "50gb", "min_index_age": "1d"}},
                        ],
                        "transitions": [
                            {"state_name": "warm", "conditions": {"min_index_age": "7d"}},
                        ],
                    },
                    {
                        "name": "warm",
                        "actions": [],
                        "transitions": [
                            {"state_name": "delete", "conditions": {"min_index_age": "30d"}},
                        ],
                    },
                    {
                        "name": "delete",
                        "actions": [{"delete": {}}],
                        "transitions": [],
                    },
                ],
            },
        },
        {
            "_id": "metrics-policy",
            "policy": {
                "policy_id": "metrics-policy",
                "description": "Metrics index lifecycle",
                "states": [
                    {
                        "name": "hot",
                        "actions": [
                            {"rollover": {"min_index_age": "7d"}},
                        ],
                        "transitions": [
                            {"state_name": "delete", "conditions": {"min_index_age": "90d"}},
                        ],
                    },
                    {
                        "name": "delete",
                        "actions": [{"delete": {}}],
                        "transitions": [],
                    },
                ],
            },
        },
    ]
}

SNAPSHOT_REPOS = {
    "s3-backups": {"type": "s3", "settings": {"bucket": "my-os-backups"}},
}

SNAPSHOT_DETAILS = [
    {
        "repository": "s3-backups",
        "snapshots": {
            "snapshots": [
                {"snapshot": "daily-2026.06.16", "state": "SUCCESS"},
                {"snapshot": "daily-2026.06.15", "state": "SUCCESS"},
            ]
        },
    }
]

NODES_USAGE = {
    "_nodes": {"total": NUM_NODES, "successful": NUM_NODES},
    "nodes": {
        f"node_{i}": {
            "rest_actions": {
                "nodes:data:read/search[phase/query]": 50000,
                "indices:data/write/bulk[s]": 30000,
                "cluster:monitor/health": 10000,
                "indices:admin/refresh[s]": 5000,
            }
        }
        for i in range(NUM_NODES)
    },
}

CAT_PLUGINS = [
    {"name": f"os-data-{i}", "component": "opensearch-security", "version": OS_VERSION}
    for i in range(NUM_NODES)
] + [
    {"name": f"os-data-{i}", "component": "opensearch-index-management", "version": OS_VERSION}
    for i in range(NUM_NODES)
]

DASHBOARDS_SAVED_OBJECTS_DASHBOARD = {
    "total": 3,
    "saved_objects": [
        {"id": f"dash-{i}", "type": "dashboard", "attributes": {"title": f"Dashboard {i}"}}
        for i in range(3)
    ],
}

DASHBOARDS_DATA_VIEWS_LIST = {
    "data_view": [
        {"id": "dv-1", "title": "logs-*", "name": "Logs"},
        {"id": "dv-2", "title": "metrics-*", "name": "Metrics"},
    ]
}

DASHBOARDS_DATA_VIEW_DETAIL_1 = {
    "data_view": {
        "id": "dv-1",
        "title": "logs-*",
        "name": "Logs",
        "timeFieldName": "@timestamp",
    }
}

DASHBOARDS_DATA_VIEW_DETAIL_2 = {
    "data_view": {
        "id": "dv-2",
        "title": "metrics-*",
        "name": "Metrics",
        "timeFieldName": "@timestamp",
    }
}

# AOSS-specific: only cat_indices and index_settings are available
AOSS_CAT_INDICES = [
    {
        "index": "logs-aoss-2026.06.16",
        "health": "green",
        "status": "open",
        "pri": "2",
        "rep": "0",
        "docs.count": "500000",
        "docs.deleted": "0",
        "store.size": "1000000000",
        "pri.store.size": "1000000000",
    }
]
